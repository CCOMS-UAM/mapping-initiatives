...
were identified through from abovementioned sources.
These initiatives are presented in \@ref(tab:initiatives),
...
so the proportion of missing information is sometimes as large as
`r max_missing_out`.
...
Some initiatives had harmonized as much as `r harm_cohorts_max_out` cohorts
while others had not harmonized their cohorts at all.
However, at least `r more_cohorts_expected_out` initiatives expected
to harmonize more cohorts in the future.
The harmonized cohorts had a large variability in the number of harmonized
variables, ranging from `r harm_vars_range_out`
...
`r max_age_max_out` (although probably referring to an unbounded upper limit),
while the most prevalent topic was that of Ageing,
