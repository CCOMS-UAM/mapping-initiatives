...
tab1_continents <- tab1_new                                                  |>
  ...
  count(id, continent)                                                       |>
  complete(id, continent, fill = list(n = 0))                                |>
...
tab1_missing_countries <- tab1_new              |>
  ...
  mutate(
    region    = region |> str_replace("American?", "Americas"), # Correct values
    continent = region |>
      countrycode(origin = "country.name", destination = "continent"),
    n         = region    |>
      str_extract("\\d+") |>
...
tab1_missing_countries <- tab1_missing_countries                 |>
  complete(id, continent, fill = list(n = 0, bool = FALSE))      |>
...
tab1_headers <- read_excel(
  ...
  slice(-10)                                                               |>
  add_column(var_name = colnames(tab1_new_out)) # Add variable names column
```

```{r create-labels}
tab1_labels <- tab1_headers                                          |>
  unite(header, subheader, col = "label", na.rm = TRUE, sep = COLON) |>
