...
```{r access-metadata-derivate_vars}
tab1_access_md <- tab1_new |> select(id, access.metadata)

tab1_access_md_headers <- tab1_access_md |>
  drop_na()                              |>
  distinct(access.metadata)              |>
  transmute(
    var_name  = access.metadata |> paste0("_metadata"),
    header    = "Metadata access",
    subheader = access.metadata                              |>
      str_replace(pattern = UNDERSCORE, replacement = SPACE) |>
      str_to_sentence()
  )

tab1_access_md <- tab1_access_md                            |>
  mutate(
    value            = TRUE,
    access.metadata  = access.metadata %>% if_else(
      condition = !is.na(.),
      true      = paste0(., "_metadata"),
      false     = NA_character_
    )
  )                                                         |>
  complete(id, access.metadata, fill = list(value = FALSE)) |>
  drop_na()                                                 |>
  pivot_wider(names_from = access.metadata, values_from = value)
```

```{r access-individual-data-derivate_vars}
tab1_access_id <- tab1_new |> select(id, access.individualdata)

tab1_access_id_headers <- tab1_access_id |>
  drop_na()                              |>
  distinct(access.individualdata)        |>
  transmute(
    var_name  = access.individualdata |> paste0("_individual"),
    header    = "Individual data access",
    subheader = access.individualdata                        |>
      str_replace(pattern = UNDERSCORE, replacement = SPACE) |>
      str_to_sentence()
  )

tab1_access_id <- tab1_access_id                                  |>
  mutate(
    value                 = TRUE,
    access.individualdata = access.individualdata %>% if_else(
      condition = !is.na(.),
      true      = paste0(., "_individual"),
      false     = NA_character_
    )
  )                                                               |>
  complete(id, access.individualdata, fill = list(value = FALSE)) |>
  drop_na()                                                       |>
  pivot_wider(names_from = access.individualdata, values_from = value)
```
...
suppressMessages( # Message when reading empty column names
  tab1_headers <- read_excel(
    ...
    add_column( # Add variable names column
      var_name = tab1_new                                                |>
        select(-c(id:region, setting:funding), -countries, -healthtopic) |>
        colnames()
      )
  ## TODO: Continue here (añadir headers para setting, harmonizationstrategy, omics, team_activity, country_institution, funding)
)

# Add health topic variables and headers/labels
tab1_new <- tab1_new                   |>
  left_join(tab1_topics,    by = "id") |>
  left_join(tab1_context,   by = "id") |>
  left_join(tab1_analysis,  by = "id") |>
  left_join(tab1_access_md, by = "id") |>
  left_join(tab1_access_id, by = "id")

tab1_headers <- tab1_headers |> bind_rows(
  tab1_topic_headers,
  tab1_context_headers,
  tab1_analysis_headers,
  tab1_access_md_headers,
  tab1_access_id_headers
)
