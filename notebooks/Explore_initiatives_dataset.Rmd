...
MICA_SHEET    <- "synchros-initiatives-20211112"
TABLE_1_SHEET <- "Table 1"

ROOT_DIR <- ".."
DATA_DIR <- "dat"
DATASET_FILENAME <- file.path(DATA_DIR, paste0(MICA_SHEET, ".xlsx"))
...
The file `r DATASET_FILENAME` has the
list of initiatives exported from the Mica repository and sent by Yannick
on 12/11/2021, 14:26 
(email subject:
"Re: Synchros - Publications Mapping of initiatives that integrate European ...
(UAM: Daniel)").
In order to better explore Table 1 from Laura's draft,
I copy and paste it to Excel,
putting it in a new sheet titled `r TABLE_1_SHEET` in that same file.
Then I read both datasets.

```{r load-data}
mica_data <- read_excel(
  DATASET_FILENAME,
  sheet = MICA_SHEET,
  na    = c("NA", "na")
)
