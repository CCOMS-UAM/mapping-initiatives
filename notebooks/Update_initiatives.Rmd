...
The columns `healthTopic` and `healthTopicOther` are additional columns that
may help provide descriptive information, so are selected as well,
even though they are not in Table 1.
...
tab1_countries <- tab1_countries_regions |>
  ...
  mutate(# Standardize country names by using their ISO3 codes:
    countries = countries                |>
      countryname(destination = "iso3c") |>
      countrycode(origin = 'iso3c', destination = "country.name")
  )                                      |>
  drop_na(countries)                     |>
  group_by(id)                           |>
...
names_correspondence <- tibble(
  repository = mica_data_tab1                                        |>
    select(-region, -ends_with("countries"), -starts_with("health")) |>
